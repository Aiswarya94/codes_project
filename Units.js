...
function Units() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://localhost:8080/getunit');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/addunit', {
        method: 'POST',
        headers: {
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/updateunit/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:8080/deleteunit/${id}`, {
        method: 'DELETE',
      });
            ...
            <TableBody>
              {data.map((item) => (
                <TableRow key={item._id}>
                  <TableCell>{item.  unitdescr}</TableCell>
                  <TableCell>{item.  unitfulln }</TableCell>
                  <TableCell>{item.  unitdescr}</TableCell>
