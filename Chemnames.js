...
function Chemnames() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://localhost:8080/getchem');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/addchem', {
        method: 'POST',
        headers: {
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/updatechem/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:8080/deletechem/${id}`, {
        method: 'DELETE',
      });
      ...
      <Typography sx={{ padding: '0 10px 0 10px' }}>
        ...
        <TableContainer component={Paper}>
          <Table>
            <TableHead>
              <TableRow>
                <TableCell>Chemnames Name</TableCell>
                <TableCell>chemengname</TableCell>
            ...
            <TableBody>
              {data.map((item) => (
                <TableRow key={item._id}>
                  <TableCell>{item. chemname}</TableCell>
                  <TableCell>{item. chemengname}</TableCell>
                  <TableCell>{item. chemdescr}</TableCell>
        ...
        <Dialog open={openDialog} onClose={handleCloseDialog}>
          ...
          <DialogContent>
            <TextField
              ...
              onChange={(e) =>
                setFormData({ ...formData,  chemname: e.target.value })
              }
            />
            <TextField
              sx={{ marginTop: '20px' }}
              label="Description"
              fullWidth
              value={formData.  chemengname}
              onChange={(e) =>
                setFormData({ ...formData,   chemengname: e.target.value })
              }
            />
