...
function ProductType () {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://localhost:8080/getmat');
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:8080/deleteproduct/${id}`, {
        method: 'DELETE',
      });
            ...
            <TableBody>
              {data.map((item) => (
                <TableRow key={item._id}>
                  ...
                  <TableCell>{item.  prodtypedescr}</TableCell>
        ...
        <Dialog open={openDialog} onClose={handleCloseDialog}>
          ...
          <DialogContent>
            <TextField
              ...
              onChange={(e) =>
                setFormData({ ...formData,  prodtypename: e.target.value })
              }
            />
            <TextField
              sx={{ marginTop: '20px' }}
              label="Description"
              fullWidth
              value={formData.  prodtypedescr}
              onChange={(e) =>
                setFormData({ ...formData,   prodtypedescr: e.target.value })
              }
            />
