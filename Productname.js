...
function Productname() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://localhost:8080/getproductname');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/addproductname', {
        method: 'POST',
        headers: {
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/updateproductname/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:8080/deleteproductname/${id}`, {
        method: 'DELETE',
      });
            ...
            <TableBody>
              {data.map((item) => (
                <TableRow key={item._id}>
                  ...
                  <TableCell>{item.  prodctdescr}</TableCell>
        ...
        <Dialog open={openDialog} onClose={handleCloseDialog}>
          ...
          <DialogContent>
            ...
            <TextField
              ...
              label="Description"
              fullWidth
              value={formData.  prodctdescr}
              onChange={(e) =>
                setFormData({ ...formData,   prodctdescr: e.target.value })
              }
            />
