...
function  States() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/addstate', {
        method: 'POST',
        headers: {
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/updatestate/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:8080/deletestate/${id}`, {
        method: 'DELETE',
      });
            ...
            <TableHead>
              <TableRow>
                <TableCell>Product Name</TableCell>
                 <TableCell>statedescr</TableCell> 
                 <TableCell> state_country</TableCell> 
                <TableCell>state_img_url</TableCell>
            ...
            <TableBody>
              {data.map((item) => (
                <TableRow key={item._id}>
                  ...
                  <TableCell>{item.  statedescr}</TableCell>
                  <TableCell>{item.  state_country}</TableCell>
                  <TableCell>{item.  state_img_url}</TableCell>
